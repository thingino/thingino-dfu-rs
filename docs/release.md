...
4. **A release that is more than a commit list has its notes written**, in
   `docs/release-notes/<tag>.md`. The publish step puts that file in front of the
   generated notes when it exists; a pre-release usually does without one.
5. **The version line says what you are about to tag:**
...
6. **A dry run has passed on this commit** (below), so the first tag run is a repeat of
...
missing. The run's own artifact list shows one more entry, `web-flasher`, which the gate
(`ci.yml`) uploads for its browser tests; it is not an archive and Gather does not count
it.
...
5. The web flasher deploys itself to GitHub Pages after the publish (`pages.yml`, called
   by the release workflow), at webflash.thingino.com once that domain is on this
   repository. To redeploy without a release, `gh workflow run pages.yml --ref main`.

6. Check what a user gets, on the archive from the release rather than the one in `dist/`:
