...
Flash Ingenic XBurst devices over USB using DFU, in Rust: the `thingino-dfu` command-line tool,
...
in one line and the run goes on. The wire is plaintext: there is no TLS, and without `--token` there is no
