...
/// The one completion line, in the tool's own words rather than the protocol's: the C
/// said `DFU upload complete: %u bytes` (`dfu.c:861`), and a user reading a flash is
/// not uploading anything. Kept in one place so a frontend can recognise it and a test
/// can pin it.
fn complete_note(total: u64) -> String {
    format!("Read complete: {total} bytes")
}
    ...
    fn op_read_an_empty_alt_reads_nothing() -> TestResult {
        ...
        assert_eq!(
            ...
            vec![
                ...
                Progress::Note("Read complete: 0 bytes".to_owned()),
            ],
    ...
    fn op_read_progress_reports_bytes_and_one_completion_line() -> TestResult {
        ...
        assert_eq!(
            ...
            vec![
                ...
                bytes(8192),
                bytes(10_000),
                Progress::Note("Read complete: 10000 bytes".to_owned()),
            ]
        );
        Ok(())
    }
