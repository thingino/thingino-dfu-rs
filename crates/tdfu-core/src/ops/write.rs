...
/// local one.
pub const COMPLETE_NOTE: &str = "Write complete";
...
/// report `Write complete` for a file with nothing in it, because nothing checks
...
mod tests {
    ...
    fn op_write_sequence() -> TestResult {
        ...
        assert_eq!(said.notes(), ["Write complete"], "said so, exactly once");
        Ok(())
    }
