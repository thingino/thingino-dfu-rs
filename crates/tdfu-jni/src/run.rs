...
/// own completion notes (`Write complete`, `Verify OK`, ...) come through the
...
mod tests {
    ...
    fn progress_reaches_the_callback_through_the_relay() {
        ...
        let operation = async {
            ...
            relay(Progress::Note("Write complete".to_owned()));
            Ok::<(), Error>(())
        };
        ...
        assert_eq!(sink.logs.borrow().as_slice(), ["Write complete"]);
    }
