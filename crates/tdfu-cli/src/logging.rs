...
/// Held by every test that installs a scoped subscriber, for as long as it is installed.
///
/// A scoped subscriber is thread-local, but two things about it are process-wide:
/// [`tracing::level_filters::LevelFilter::current`] is the most verbose level any live
/// subscriber asks for, and the interest `tracing` caches per callsite is rebuilt over
/// all of them. A `-d` subscriber alive on one test thread therefore makes a quiet
/// subscriber on another read as `DEBUG`. Tests in this binary that install one take
/// this lock first, so no two are alive at once.
#[cfg(test)]
pub(crate) static SCOPED_SUBSCRIBER: std::sync::Mutex<()> = std::sync::Mutex::new(());
...
mod tests {
    ...
    fn the_installed_subscriber_carries_the_level_the_flag_asks_for() {
        ...
        let _alone = super::SCOPED_SUBSCRIBER
            .lock()
