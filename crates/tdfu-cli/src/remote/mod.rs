...
pub async fn run<C: Sleeper>(
    ...
    // same trade.
    session.say(&format!("Connected to the dfu-remote daemon at {}.", session.at), err);
    if plan.debug {
        session
            .ask_for_narration(err)
            .map_err(|error| error.failure(OpClass::Remote))?;
    }
...
impl Session<'_> {
    ...
    fn read(&mut self, err: &mut dyn Write) -> Result<(), RemoteError> {
        ...
        // `Read complete: N bytes` into the log stream and this client renders it
    ...
    ///
    /// Locally, `-d` shows core narrating the protocol (which alt it claimed, how the
    /// download is cut into blocks) between the CLI's own diagnostics. Remotely that
    /// core runs in the daemon, so the daemon is asked to send it: `CMD_DEBUG` is
    /// connection state on the far side, answered once, and every command after it on
    /// this connection carries the narration as `RESP_DEBUG` frames, which
    /// [`Client::finish`] renders through the same path a local run uses.
    ///
    /// A daemon that predates the command refuses it with `unknown command` and keeps
    /// the connection. That is a note rather than a failure: the run is still worth
    /// having, it just narrates one side of the wire. Anything else wrong with the
    /// answer is the protocol class, as for every command.
    fn ask_for_narration(&mut self, err: &mut dyn Write) -> Result<(), RemoteError> {
        self.send(&Request::Debug, doing::NARRATION)?;
        match self.finish(doing::NARRATION, err) {
            Ok(_empty) => Ok(()),
            Err(RemoteError::Refused(message)) if message.ends_with(": unknown command") => {
                self.say(
                    &format!(
                        "note: the daemon at {} predates the narration frames, so -d shows this side of the wire \
                         only; update the daemon to see its core narrate too",
                        self.at
                    ),
                    err,
                );
