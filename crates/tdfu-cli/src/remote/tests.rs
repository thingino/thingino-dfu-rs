...
fn rpc_cli_remote_list() -> TestResult {
    ...
    assert!(
        !outcome.out.contains("Connected to"),
...
fn rpc_read_streams_to_the_file() -> TestResult {
    ...
    // The client's own leg has its own name: the daemon's `reading` counter is over by
    // the time the image streams to the file, and this counter must not borrow it.
    assert!(
        outcome.err.contains("receiving  100%  200000/200000 bytes"),
        "{}",
        outcome.err
    );
    assert!(!outcome.err.contains("reading"), "{}", outcome.err);
...
fn rpc_progress_draws_the_bar_and_a_log_line_terminates_it() -> TestResult {
    ...
    let complete = outcome.err.find("Write complete").ok_or("the log line was dropped")?;
...
fn a_bad_progress_frame_does_not_kill_a_completed_transfer() -> TestResult {
    ...
    assert!(
        outcome.err.contains("writing   25%"),
...
// ---------------------------------------------------------------------------
// -d over the wire: the daemon's narration
// ---------------------------------------------------------------------------

/// Run `work` under a `-d` subscriber that captures the diagnostics, so a test can see
/// what `-d` would have printed as well as what the run wrote to stderr. `work` is
/// handed a snapshot of what has been captured so far.
///
/// Scoped to this thread and this closure (`with_default`), never global, and alone:
/// see [`logging::SCOPED_SUBSCRIBER`](crate::logging::SCOPED_SUBSCRIBER) for what a
/// second live subscriber would do to the other tests in this binary.
fn diagnostics_of<T>(work: impl FnOnce(&dyn Fn() -> String) -> T) -> (T, String) {
    ...
    let _alone = crate::logging::SCOPED_SUBSCRIBER
        .lock()
        .unwrap_or_else(PoisonError::into_inner);
    let captured = Capture(Arc::new(Mutex::new(Vec::new())));
    let writer = captured.clone();
    let subscriber = tracing_subscriber::fmt()
        .with_max_level(crate::logging::VERBOSE)
        .with_target(true)
        .without_time()
    ...
    let value = tracing::subscriber::with_default(subscriber, || work(&so_far));
    (value, captured.text())
}

/// Render one narration line and one phase change locally, under the subscriber in
/// place, until both are seen.
///
/// `tracing` caches an interest per callsite and rebuilds the cache when a subscriber is
/// registered. Another test thread first hitting the narration callsite with no
/// subscriber caches "never", and when that first hit races the registration here the
/// stale "never" survives it: this thread's subscriber then hears nothing from that
/// callsite, which is how the parity test below failed once under the full workspace
/// run and passed alone. Rendering until the line is seen, rebuilding the cache between
/// tries, settles the callsite for as long as this subscriber lives.
///
/// Returns the two lines as captured, the narration and the phase, so the remote lines
/// can be compared against them.
fn narration_callsites_settled(so_far: &dyn Fn() -> String) -> (String, String) {
    const WARM: &str = "warming the narration callsite";
    for _ in 0..10_000 {
        let mut bar = crate::render::Bar::new();
        bar.render(&tdfu_core::Progress::Debug(WARM.to_owned()), &mut Vec::new());
        bar.render(
            &tdfu_core::Progress::Phase(tdfu_core::progress::Phase::Stage1),
            &mut Vec::new(),
        );
        let seen = so_far();
        let narration = seen.lines().find(|line| line.ends_with(WARM));
        let phase = seen.lines().find(|line| line.ends_with("phase phase=Stage1"));
        if let (Some(narration), Some(phase)) = (narration, phase) {
            return (narration.to_owned(), phase.to_owned());
        }
...
/// `-d` asks the daemon to narrate before anything else is sent, and a run without
/// `-d` never asks (`rpc_cli_remote_flow_runs_the_whole_plan_in_order` pins that
/// command list).
#[test]
fn a_debug_run_asks_the_daemon_to_narrate_before_anything_else() -> TestResult {
    let daemon = FakeDaemon::start(
        false,
        vec![
            vec![Step::Ok(Vec::new())], // DEBUG: answered on its own, empty
            vec![Step::Ok(discovered(2, WireVariant::UNKNOWN))],
        ],
    )?;
    let plan = plan_for(daemon.port(), &["-d", "-l"])?;
    let outcome = drive(&plan);
    let transcript = daemon.transcript()?;
    ...
    assert_eq!(
        commands,
        vec![Command::Debug.wire_byte(), Command::Discover.wire_byte()],
        "the ask goes first, once"
    );
    assert!(
        !outcome.err.contains("predates"),
        "a daemon that answered the ask is not an old one: {}",
        outcome.err
    );
    Ok(())
}

/// A daemon older than the ask refuses it with `unknown command` and keeps the
/// connection, and the run goes on with a note: one side of the wire narrating is
/// better than no run.
#[test]
fn a_daemon_that_predates_the_narration_is_a_note_not_a_failure() -> TestResult {
    let daemon = FakeDaemon::start(
        false,
        vec![
    ...
    assert!(outcome.result.is_ok(), "{:?}", outcome.refusal());
    assert!(outcome.err.contains("predates the narration frames"), "{}", outcome.err);
    let commands: Vec<u8> = transcript.requests.iter().map(|(command, _)| *command).collect();
    assert_eq!(
        commands,
        vec![Command::Debug.wire_byte(), Command::Discover.wire_byte()]
...
/// Any other trouble with the ask is the wire failing, and it is reported as such.
#[test]
fn a_connection_lost_on_the_ask_is_a_protocol_failure() -> TestResult {
    let daemon = FakeDaemon::start(false, vec![vec![Step::Close]])?;
    let plan = plan_for(daemon.port(), &["-d", "-l"])?;
    let outcome = drive(&plan);
    let _transcript = daemon.transcript_raw()?;

    let (message, code) = outcome.refusal()?;
    assert_eq!(code, PROTOCOL, "{message}");
    assert!(message.contains("the narration request"), "{message}");
    Ok(())
}

/// **The parity pin.** A `RESP_DEBUG` line is rendered exactly as core's own narration
/// is on a local run: through `tracing` at debug level, with the same target, and never
/// on stderr where the bar and the logs are. A phase change on the wire is said the way
/// a local `Progress::Phase` is said, too.
#[test]
fn the_daemons_narration_reads_as_the_local_narration_does() -> TestResult {
    let daemon = FakeDaemon::start(
        false,
        vec![
            vec![Step::Ok(Vec::new())],
            ...
            vec![
                Step::Progress {
                    percent: 0,
                    stage: tdfu_core::progress::Phase::Stage1.wire_byte(),
                    message: "stage1".to_owned(),
                },
                Step::Debug("claiming alt 0 on interface 0".to_owned()),
                Step::Log("rebooting".to_owned()),
                Step::Ok(Vec::new()),
            ],
        ],
    )?;
    let plan = plan_for(daemon.port(), &["-d", "--reboot"])?;
    let ((local, phase, outcome), diagnostics) = diagnostics_of(|so_far| {
        // The local rendering of a narration line and a phase change, under this
        // subscriber, with the callsites settled before the run (see
        // `narration_callsites_settled`).
        let (local, phase) = narration_callsites_settled(so_far);
        (local, phase, drive(&plan))
    });
    daemon.transcript()?;
    assert!(outcome.result.is_ok(), "{:?}", outcome.refusal());
    assert!(!local.is_empty(), "the local narration must reach the diagnostics");
    assert!(!phase.is_empty(), "the local phase line must reach the diagnostics");

    // The remote line is the local line with the text swapped, byte for byte: same
    // level, same target, same shape.
    let expected = local.replace("warming the narration callsite", "claiming alt 0 on interface 0");
    assert_ne!(expected, local);
    assert!(
        diagnostics.lines().any(|line| line == expected),
        "the remote line must be the local line, byte for byte:\n{diagnostics}\n---\n{expected}"
    );
    // The phase line is the local one, byte for byte, and said once for the frame.
    assert_eq!(
        diagnostics.lines().filter(|line| *line == phase).count(),
        2,
        "the local warm-up and the wire, and nothing else:\n{diagnostics}"
    );

    // And stderr carries the log line and not the narration, as locally.
    assert!(outcome.err.contains("rebooting"), "{}", outcome.err);
    assert!(
        !outcome.err.contains("claiming alt 0"),
