...
impl Client {
    ...
    pub fn read_to(
        ...
        while received < data_len {
            ...
            // This client's own leg: the daemon has read the device (its counter said
            // `reading`, and its note said the read was complete), and what moves now
            // is the image, from the daemon to this file.
            bar.counter("receiving", received, Some(data_len), err);
        }
        out.flush()
            .map_err(|source| RemoteError::file(format!("cannot write to {}", path.display()), &source))?;
    ...
    /// Render `RESP_LOG`, `RESP_PROGRESS` and `RESP_DEBUG` frames until a final one
    /// arrives, or until the peer has sent [`MAX_INTERMEDIATE_FRAMES`] of them without one.
    ///
    ...
    fn until_final(&mut self, doing: &str, bar: &mut Bar, err: &mut dyn Write) -> Result<ResponseHeader, RemoteError> {
        ...
        loop {
            ...
            if matches!(header.status, Status::Log | Status::Progress | Status::Debug) {
                intermediates += 1;
                if intermediates > self.intermediate_budget {
                    return Err(RemoteError::protocol(format!(
                        "the daemon at {} has sent {intermediates} log, progress and debug frames during {doing} \
                         and no final answer; a command that never ends cannot be waited out, so this client is \
                         giving up on it",
                        self.at
                    )));
                }
            ...
            match header.status {
                ...
                // The daemon's core narrating, one line per frame, because this
                // connection asked (`CMD_DEBUG`, sent by `-d`). It is rendered by the
                // path core's own lines take on a local run, [`Bar::render`] with a
                // `Progress::Debug`: to `tracing` at debug level, never to `err`, so a
                // remote `-d` reads as a local one and a bar mid-draw is not disturbed.
                Status::Debug => {
                    let payload = self.intermediate_payload(header.payload_len, doing)?;
                    let text = crate::render::sanitise(&String::from_utf8_lossy(&payload));
                    bar.render(&tdfu_core::Progress::Debug(text.trim_end_matches('\n').to_owned()), err);
                }
                _ => return Ok(header),
            }
        }
    ...
    /// [`Status`] has five members, and named rather than assumed if that changes.
...
fn status_name(status: Status) -> &'static str {
    match status {
        ...
        // saying so beats calling it one of the five above.
        _ => "unrecognised",
    }
}
...
mod tests {
    ...
    fn a_daemon_that_never_answers_is_given_up_on() -> TestResult {
        ...
        assert!(
            message.contains("5 log, progress and debug frames"),
    ...
    /// Every response kind has a word, and they are five different words.
    ///
    /// Only `progress` is reachable through a daemon — the other four are consumed
    ...
    fn every_status_has_its_own_name() {
        ...
        assert_eq!(names, vec!["ok", "error", "progress", "log", "debug"]);
        let mut unique = names.clone();
        unique.sort_unstable();
        unique.dedup();
        assert_eq!(unique.len(), names.len(), "five kinds, five words");
    }
