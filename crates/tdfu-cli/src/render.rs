...
///
/// Every completion line — `Write complete`, `Verify OK: N bytes match`,
/// `Read complete: N bytes`, both retry announcements — is emitted by
...
pub struct Bar {
    ...
    last: Option<u64>,
    /// The stage byte of the last `RESP_PROGRESS` frame, so [`wire`](Bar::wire) can say
    /// when the phase changes, the way [`render`](Bar::render) says it for a local
    /// [`Progress::Phase`]. `None` before the first frame.
    stage: Option<u8>,
}
...
impl Bar {
    ...
    pub fn render(&mut self, progress: &Progress, err: &mut dyn Write) {
        match progress {
            ...
            Progress::Phase(phase) => {
                ...
                // from zero again.
                self.last = None;
                self.diagnostic(err, || tracing::debug!(?phase, "phase"));
            }
            ...
            // is where the bar draws; with `-d` on, the counter makes way for it first.
            Progress::Debug(text) => self.diagnostic(err, || tracing::debug!("{text}")),
            ...
            // be user-facing.
            other => self.diagnostic(err, || tracing::debug!(?other, "progress")),
        }
    }

    /// One diagnostic line, from a place where a counter may be on the screen.
    ///
    /// The counter is drawn with a carriage return and no newline, so a line the
    /// `tracing` subscriber writes to the same stderr lands right after it:
    /// `stage1    0%DEBUG tdfu_cli::render: stage 1: 10944 bytes to …`. With `-d` on the
    /// counter is blanked first and the next byte count redraws it. With `-d` off the
    /// subscriber drops the line, so the counter is left exactly as it was: a quiet run
    /// never blanks or flickers for a line nobody sees.
    fn diagnostic(&mut self, err: &mut dyn Write, emit: impl FnOnce()) {
        if self.drawn > 0 && tracing::enabled!(tracing::Level::DEBUG) {
            self.clear(err);
        }
        emit();
    }
    ...
    fn bytes(&mut self, phase: Phase, done: u64, total: Option<u64>, err: &mut dyn Write) {
        self.counter(&phase.to_string(), done, total, err);
    }

    /// Draw a byte counter under a label of the caller's: the remote client's own leg of
    /// a read, `receiving   45%  4718592/10485760 bytes`, which is not one of core's
    /// phases (the daemon has finished reading the device by then) and must not borrow
    /// one's name.
    pub fn counter(&mut self, label: &str, done: u64, total: Option<u64>, err: &mut dyn Write) {
        ...
        self.paint(&Self::line(label, done, total), err);
    }
    ...
    pub fn wire(&mut self, percent: u8, stage: u8, message: &str, err: &mut dyn Write) {
        // The same diagnostic line a local run's `Progress::Phase` produces, so `-d`
        // reads alike on both sides; an unknown byte is reported as itself.
        if self.stage != Some(stage) {
            self.stage = Some(stage);
            self.diagnostic(err, || {
                if let Some(phase) = wire_phase(stage) {
                    tracing::debug!(?phase, "phase");
                } else {
                    tracing::debug!(stage, "phase");
                }
            });
        }
    ...
    /// `writing   45%  4718592/10485760 bytes`, in the wire's smaller vocabulary:
    /// `erasing   50%  3 blocks erased`.
    ///
    ...
    /// transition as `writing    0%  writing`.
    fn wire_line(percent: u8, stage: u8, message: &str) -> String {
        ...
        // A phase's opening frame carries nothing but its name and a percent of 0, so
        // it is announced by the name alone: `finishing` rather than `finishing    0%`,
        // which reads as a measurement of nothing.
        if percent == 0 && message == phase {
            return phase;
        }
    ...
    /// `writing   45%  4718592/10485760 bytes`, or `reading  8388608 bytes`.
    ///
...
"
    ...
    fn a_note_survives_the_bar_that_was_drawing() -> TestResult {
        let text = drawn(&[
            ...
            Progress::Note("Write complete".to_owned()),
        ])?;
        assert!(
            text.ends_with("Write complete\n"),
            "the note must be the last thing said, on its own line: {text:?}"
        );
        assert_eq!(text.matches("Write complete").count(), 1, "{text:?}");
        Ok(())
    }
    ...
    fn a_debug_line_is_narration_and_never_reaches_the_bar() -> TestResult {
        ...
        // And on a quiet run it does not disturb a counter mid-draw: the bar after it
        // is the bar before it, byte for byte. (No subscriber is installed here, which
        // is what a run without `-d` looks like to the bar.)
    ...
    /// **With `-d` on, a narration line does not land mid-counter.** The counter is
    /// blanked before the line goes to the subscriber, and the next byte count draws it
    /// again. Before this, a `-d` run read `stage1    0%DEBUG tdfu_cli::render: stage 1:
    /// …` on one terminal line. Revert check: drop the `clear` in `Bar::diagnostic` and
    /// the blank between the two draws disappears.
    #[test]
    fn under_debug_a_narration_line_blanks_the_counter_first() -> TestResult {
        ...
        tracing::subscriber::with_default(subscriber, || -> TestResult {
            ...
            let line = "writing   50%  4096/8192 bytes";
            let paint = format!("\r{line}");
            let blank = format!("\r{:width$}\r", "", width = line.len());

            // The `enabled!` check inside `Bar::diagnostic` has a callsite whose cached
            // interest can lag a registration on another thread (see
            // `remote::tests::narration_callsites_settled`); rendering until the blank
            // is seen settles it.
            let mut seen = String::new();
            for _ in 0..10_000 {
                seen = drawn(&[counting.clone(), narration.clone()])?;
                if seen.contains(&blank) {
                    break;
                }
                tracing::callsite::rebuild_interest_cache();
                std::thread::yield_now();
            }
            assert_eq!(seen, format!("{paint}{blank}"), "the counter is blanked, once");

            // And the next byte count draws it again, so the counter is not lost.
            assert_eq!(
                drawn(&[counting.clone(), narration, counting])?,
                format!("{paint}{blank}{paint}{blank}"),
                "blanked before the line, redrawn after it, and blanked once more at the end"
            );
            Ok(())
        })
    }

    /// A counter under a label of the caller's draws like a phase's, throttled the same
    /// way, and says the label: the remote client's own `receiving` leg.
    #[test]
    fn a_labelled_counter_draws_like_a_phase() -> TestResult {
        let mut bar = Bar::new();
        let mut err = Vec::new();
        bar.counter("receiving", 4_718_592, Some(10_485_760), &mut err);
        bar.counter("receiving", 4_718_592, Some(10_485_760), &mut err);
        bar.counter("receiving", 10_485_760, Some(10_485_760), &mut err);
        let text = String::from_utf8(err)?;
        assert!(text.contains("receiving   45%  4718592/10485760 bytes"), "{text:?}");
        assert!(text.contains("receiving  100%  10485760/10485760 bytes"), "{text:?}");
        assert_eq!(text.matches('\r').count(), 2, "the identical frame is not redrawn");
        Ok(())
    }
    ...
    fn a_counter_with_no_total_redraws_once_a_mebibyte() -> TestResult {
        ...
        let text = drawn(&steps)?;
        assert_eq!(
            text.matches("reading").count(),
            2,
            "one draw below the mebibyte and one above it: {text:?}"
        );
        assert!(text.contains("reading  4096 bytes"), "{text:?}");
        assert!(text.contains("reading  1048576 bytes"), "{text:?}");
        assert!(
    ...
    fn a_wire_progress_frame_draws_the_local_counter() -> TestResult {
        ...
        assert!(text.contains("writing   45%  4718592/10485760 bytes"), "{text:?}");
    ...
    /// transition read `writing    0%  writing`.
    #[test]
    fn a_phase_frame_does_not_say_the_phase_twice() -> TestResult {
        ...
        let text = String::from_utf8(err)?;
        assert_eq!(text.matches("writing").count(), 1, "{text:?}");
        assert!(
            !text.contains('%'),
            "a phase's opening frame is its name alone, not a measurement: {text:?}"
        );

        // A message that merely *contains* the phase name is still a message.
        let mut err = Vec::new();
        Bar::new().wire(0, Phase::Download.wire_byte(), "write stalled", &mut err);
        let text = String::from_utf8(err)?;
        assert!(text.contains("writing    0%  write stalled"), "{text:?}");
        Ok(())
    }
    ...
    fn a_wire_counter_at_a_fixed_percent_still_redraws() -> TestResult {
        ...
        assert!(text.contains("erasing    0%  3 blocks erased"), "{text:?}");
        Ok(())
    }
    ...
    fn a_wire_message_cannot_break_the_line_it_is_drawn_on() -> TestResult {
        ...
        let text = String::from_utf8(err)?;
        assert!(text.contains("erasing   50%  erasing^Jblock 1^[[2J"), "{text:?}");
