    ...
    C: Sleeper,
{
    ...
    /// `Read complete: N bytes` as a [`Progress::Note`] and this renders every
...
mod tests {
    ...
    fn fe_cli_op_order_is_the_order_operations_run_in() -> TestResult {
        ...
        let write = at(&err, "Write complete")?;
        let verify = at(&err, "Verify OK: 4200 bytes match")?;
        let read = at(&err, "Read complete: 8192 bytes")?;
    ...
    fn op_verify_follows_the_write_it_belongs_to() -> TestResult {
        ...
        assert!(at(&err, "Write complete")? < at(&err, "Verify OK")?, "{err}");
    ...
    ///
    /// `ops::read` emits `Read complete: N bytes` itself, and this arm prints no
    ...
    fn op_read_writes_the_file_and_says_the_count_once() -> TestResult {
        ...
        assert_eq!(
            err.matches("Read complete: 8192 bytes").count(),
            1,
    ...
    fn op_read_size_caps_the_file() -> TestResult {
        ...
        assert!(err.contains("Read complete: 100 bytes"), "{err:?}");
        Ok(())
    }
    ...
    fn fe_cli_autobootstrap_writes_to_the_gadget_it_created() -> TestResult {
        ...
        assert!(err.contains("Bootstrapping device 0"), "{err}");
        assert!(
            at(&err, "The U-Boot DFU gadget is up")? < at(&err, "Write complete")?,
