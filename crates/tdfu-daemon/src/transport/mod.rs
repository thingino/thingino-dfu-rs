...
impl Conn {
    ...
    /// `Write completeVerify OK: …` on one line. Terminating it **here** rather
    ...
    /// One `RESP_DEBUG` frame: a line of core's protocol narration, for a connection
    /// that asked for it. Gated twice: the connection must have asked (`CMD_DEBUG`, or
    /// `X-Debug: 1` on an HTTP request), and a command must be in flight for the frame
    /// to ride on. **Not** by the attach rule `log` and `progress` follow: that rule is
    /// for clients that asked for nothing beyond the final frame, and a client that
    /// asked for the narration reads it on every command, as a local `-d` shows it.
    ///
    /// # Errors
    /// [`DaemonError::OversizeResponse`] if the line does not fit a frame, or an I/O
    /// failure.
    pub async fn debug(&mut self, line: &str) -> Result<(), DaemonError> {
        if !(self.narrates() && self.current().is_some()) {
            return Ok(());
        }
        let terminator: &[u8] = if line.ends_with('\n') { b"" } else { b"\n" };
        let payload_len = line.len().saturating_add(terminator.len());
    ...
    /// Has this connection asked for the protocol narration?
    #[must_use]
    pub const fn narrates(&self) -> bool {
        match self {
            Self::Raw(raw) => raw.narrate,
            Self::Ws(ws) => ws.narrate,
            Self::Http(http) => http.narrate,
        }
    }

    /// Record the ask (`CMD_DEBUG`): narration frames from now on, on this connection.
    pub fn set_narration(&mut self, on: bool) {
        match self {
            Self::Raw(raw) => raw.narrate = on,
            Self::Ws(ws) => ws.narrate = on,
            Self::Http(http) => http.narrate = on,
        }
    }
