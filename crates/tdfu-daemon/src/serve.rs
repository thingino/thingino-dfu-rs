...
impl Wire for Conn {
    ...
    async fn debug(&mut self, line: &str) -> Result<(), DaemonError> {
        Conn::debug(self, line).await
    }
...
mod tests {
    ...
    async fn a_chatty_command_reaches_the_client_through_a_real_conn() -> TestResult {
        ...
        assert!(!kinds.contains(&Status::Debug), "narration nobody asked for: {kinds:?}");
        Ok(())
    }

    /// `CMD_DEBUG`, and then the narration, through a real socket.
    ///
    /// The ask is answered on its own, as connection state rather than as work, and
    /// every frame the bootstrap after it produces is read in order: the narration
    /// arrives as `RESP_DEBUG` frames, one whole line each, among the logs and before
    /// the final frame. The test above is the twin that never asks and sees none.
    #[tokio::test]
    async fn the_narration_reaches_a_client_that_asked_through_a_real_conn() -> TestResult {
        let listener = TcpListener::bind("127.0.0.1:0").await?;
        let address = listener.local_addr()?;
        let auth = Auth::open();
        let mut state = bootstrapping_daemon();
        ...
        let client = async {
            let mut stream = TcpStream::connect(address).await?;
            stream.write_all(&frame_of(&Request::Debug)?).await?;
            let (status, payload) = one_frame(&mut stream).await?;
            assert_eq!(
                (status, payload.as_slice()),
                (Status::Ok, &b""[..]),
                "the ask is answered on its own"
            );
            ...
            loop {
                ...
                if status == Status::Debug {
                    ...
                    assert!(line.ends_with('\n'), "one whole line per frame: {line:?}");
                    narration.push(line);
                }
                if !matches!(status, Status::Log | Status::Progress | Status::Debug) {
                    break;
                }
            }
            let _ignored = stop.send(());
            Ok::<_, Box<dyn std::error::Error>>((kinds, narration))
        };
        let (stopped, outcome) = tokio::join!(bounded(server), client);
        stopped?;
        let (kinds, narration) = outcome?;
