...
async fn family_agrees<C, T>(device: &T, clock: &C, requested: Variant) -> Result<Option<String>, Error>
where
    C: Sleeper,
    T: tdfu_usb::LocalUsbTransport,
{
    ...
    match detection {
        Detection::Resolved(resolved) if resolved.variant != requested => Ok(Some(format!(
    ...
    T: tdfu_usb::LocalUsbTransport,
{
    ...
    let (variant, detected, caveat) = if let Some(variant) = requested {
        ...
        //
        // A caveat qualifies a *detection*, and there was none to qualify here.
        let caveat = family_agrees(device, &state.clock, variant).await?;
        (variant, None, caveat)
    } else {
        {
...
mod tests {
    ...
    /// ... and a `--cpu` the caller supplied is **not** remembered: a claim is not a
    /// detection, and republishing it through `DISCOVER` would be the C's
    /// guess-as-fact with an extra step.
    #[test]
