...
pub trait Wire {
    ...
    /// that asked for it and has a command in flight. A no-op otherwise, and never under
    /// the attach rule: the narration rides every command on a connection that asked.
    ///
    /// # Errors
    /// [`DaemonError`] when the frame cannot be written.
