...
pub enum Sent {
    ...
    /// A `RESP_LOG` line.
    Log(String),
    /// A `RESP_DEBUG` line: narration, for a connection that asked.
    Debug(String),
...
pub struct LoopbackConn {
    ...
    /// three long ones.
    http: bool,
    /// Has this connection asked for the narration (`CMD_DEBUG` / `X-Debug`)?
    narrate: bool,
...
impl LoopbackConn {
    ...
    /// A connection that asked for the narration (`CMD_DEBUG`, or `X-Debug: 1`).
    #[must_use]
    pub fn narrating(mut self) -> Self {
        self.narrate = true;
        self
    }
...
impl Wire for LoopbackConn {
    ...
    /// Gated as `Conn::debug` is: the connection must have asked, and a command must be
    /// in flight. The attach rule does not apply to the narration.
    async fn debug(&mut self, line: &str) -> Result<(), DaemonError> {
        ...
        if !(self.narrate && self.current.get().is_some()) {
            self.suppressed.borrow_mut().push(Sent::Debug(line));
            return Ok(());
        }
        self.record(Sent::Debug(line))
    }
...
mod tests {
    ...
    fn a_log_frame_is_recorded_as_the_wire_carries_it() -> TestResult {
        let mut writing = LoopbackConn::raw().during(Command::Write);
        block_on(writing.log("Write complete"))?;
        block_on(writing.log("already terminated\n"))?;
        assert_eq!(
            writing.sent(),
            vec![
                super::Sent::Log("Write complete\n".to_owned()),
                super::Sent::Log("already terminated\n".to_owned()),
            ]
        );
        assert_eq!(writing.log_lines(), vec!["Write complete", "already terminated"]);
        Ok(())
    }
