...
async fn rpc_preflight_headers() -> TestResult {
    ...
    for header in [
        ...
        "Access-Control-Allow-Headers: Content-Type, X-Auth-Token, X-Debug",
...
async fn the_preflight_answers_the_flasher_and_refuses_the_rest() -> TestResult {
    ...
    assert!(
        reply.contains("Access-Control-Allow-Headers: Content-Type, X-Auth-Token, X-Debug\r\n"),
...
async fn rpc_progress_frames_are_sent() -> TestResult {
    ...
    assert_eq!(body.message, "writing");
...
/// printed `Write completeVerify OK: 16777216 bytes match` on one line. Checked
...
async fn rpc_log_frames_are_whole_lines() -> TestResult {
    ...
    assert_eq!(got[0].1, b"Write complete\n", "the terminator is added");
