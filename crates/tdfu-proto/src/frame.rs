...
pub enum Command {
    ...
    Reboot = 8,
    /// Send this connection the daemon's protocol narration, as [`Status::Debug`] frames,
    /// from now on. Header-only. Over HTTP the request header `X-Debug: 1` says the same
...
/// The five response kinds: the C's four, and `Debug`.
#[repr(u8)]
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
#[non_exhaustive]
pub enum Status {
    ...
    Log = 3,
    /// One whole line of the daemon's protocol narration, the same text the local
    /// debug switch prints. Sent only to a connection that asked with [`Command::Debug`]
    /// (or `X-Debug: 1` over HTTP), and only on the commands whose responses carry logs
    /// at all, so a client that never asked never sees one.
    Debug = 4,
}

impl Status {
    /// Every status, for exhaustive tests.
    pub const ALL: [Self; 5] = [Self::Ok, Self::Error, Self::Progress, Self::Log, Self::Debug];
...
mod tests {
    ...
    fn rpc_status_bytes_round_trip() -> Result<(), crate::ProtoError> {
        ...
        assert!(Status::from_wire_byte(5).is_err());
        Ok(())
    }
