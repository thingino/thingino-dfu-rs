...
Copied from the C repository's `worker/` on 2026-09-03, with one change: the tag
allow-list admits every `<branch>-<date>` release tag (the flasher's branch menu), not
only `firmware-<date>`. It is deployed from here from now on; the live Worker on the
thingino Cloudflare account still runs the old rule until someone runs the deploy
below, and until then the page lists branch builds that the proxy refuses to serve.
