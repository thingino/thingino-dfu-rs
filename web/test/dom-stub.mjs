...
class El {
    ...
    /* Only the listeners on this element run: nothing bubbles, because the page
     * listens on the overlay and reads `event.target` to tell the backdrop from
     * the card, and that is the one shape a test needs to hand it. */
    dispatchEvent(event) {
...
/* The listeners `app.js` puts on the document itself (the Escape key). */
...
export const dom = {
    ...
    /** Press one key, as the browser reports it to the document: `dom.press('Escape')`. */
    press(key) {
        (docListeners.get('keydown') || []).forEach((fn) => fn({ key }));
    },

    /** Click `el`, with the click landing on `target` (the element itself when omitted). */
    click(el, target) {
        el.dispatchEvent({ type: 'click', target: target || el });
    },
