 ...
 *
 * And the narration extension: the `X-Debug: 1` request header that asks for it,
 * and the RESP_DEBUG frames it answers with.
 */
...
/** A client with its three sinks captured. */
function client(resolver) {
    ...
    const c = new RemoteClient(
        ...
        (line) => debugs.push(line)
    );
    return { c, logs, bars, debugs };
}
...
test('narration is asked for by a header, and only when it is wanted', async () => {
    /* The daemon interleaves its protocol narration into a response only for a
     * request that asked for it, so a page with the toggle off pays nothing. */
    let seen = scriptDaemon([frame(RESP_OK, new Uint8Array(0))]);
    let { c } = client();
    c.debug = true;
    await c.connect('192.0.2.10:5050', '');
    await c.reboot(0);
    assert.equal(seen.headers[0]['X-Debug'], '1');

    seen = scriptDaemon([frame(RESP_OK, new Uint8Array(0))]);
    ({ c } = client());
    await c.connect('192.0.2.10:5050', '');
    await c.reboot(0);
    assert.equal('X-Debug' in seen.headers[0], false, 'a client that did not ask sends no header at all');
});

test('a narration frame is a line to show, not the end of the command', async () => {
    scriptDaemon([
        frame(RESP_DEBUG, 'GETSTATUS round 1: state=dfuDNLOAD-IDLE poll=5ms\n'),
        frame(RESP_LOG, 'writing 4096 bytes\n'),
        frame(RESP_DEBUG, 'download block 0: 4096 bytes'),
        frame(RESP_OK, new Uint8Array(0)),
    ]);
    const { c, logs, debugs } = client();
    c.debug = true;
    await c.connect('192.0.2.10:5050', '');

    assert.equal(await c.reboot(0), true, 'the command still ends on its own final frame');
    assert.deepEqual(
        debugs,
        ['GETSTATUS round 1: state=dfuDNLOAD-IDLE poll=5ms', 'download block 0: 4096 bytes'],
        'a narration line loses one terminator, exactly as a log line does'
    );
    assert.deepEqual(logs, ['writing 4096 bytes'], 'and it does not land in the log sink');
});

test('a narration frame nobody asked for does not break the command', async () => {
    /* Nothing stops a daemon from sending one, and the stream carries the answer
     * behind it: a status this client did not expect must not read as an error. */
    scriptDaemon([
        frame(RESP_DEBUG, 'claimed alt 0 (flash)'),
        frame(RESP_OK, new Uint8Array([1, 2, 0xa1, 0x08, 0xc3, 0x09, 0x00, 0x02])),
    ]);
    const { c, logs } = client();
    await c.connect('192.0.2.10:5050', '');

    const devices = await c.discover();
    assert.equal(devices.length, 1, 'the frame behind the narration is still the answer');
    assert.deepEqual(logs, [], 'and nothing was reported as a failure');

    // A client built with no narration sink at all drops the line instead.
    scriptDaemon([frame(RESP_DEBUG, 'claimed alt 0 (flash)'), frame(RESP_OK, new Uint8Array(0))]);
    const bare = new RemoteClient();
    await bare.connect('192.0.2.10:5050', '');
    assert.equal(await bare.reboot(0), true);
});
...
test('a progress frame is drawn with its stage', async () => {
    ...
    assert.deepEqual(bars, [
        [0, 'writing: 0 / 16777216 bytes', 'writing'],
        [50, 'verifying: 8388608 / 16777216 bytes', 'verifying'],
        [99, 'nearly there', ''],
