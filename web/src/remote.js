 ...
 * replies with a chunked stream of TDFU response frames (progress/log, plus
 * debug narration when the request asked for it, then OK/ERROR), which we parse
 * as a byte stream. fetch() is used (not WebSocket) because Chrome's Local
 * Network Access only exempts fetch({targetAddressSpace: 'local'}) from
 * mixed-content blocking - so the HTTPS flasher can reach a local/LAN daemon
 * over plain http:// with a one-time permission, no TLS.
 */
...
const RESP_LOG = 0x03;
/* Protocol narration: the core's own debug lines (GETSTATUS poll rounds, alt
 * claiming, descriptor lines, one per downloaded block). The payload is one
 * UTF-8 line, unterminated by a NUL, exactly as RESP_LOG carries one. The daemon
 * interleaves these only into a response whose request asked for them with the
 * X-Debug header below, on every command. */
 ...
 * carry here.) */
const STAGE_NAMES = ['', 'stage1', 'u-boot', 'writing', 'finishing', 'reading', 'verifying', 'erasing'];
...
export class RemoteClient {
    constructor(onLog, onProgress, variantResolver, onDebug) {
        this.url = '';
        this.token = '';
        this.onLog = onLog || function () {};
        this.onProgress = onProgress || function () {};
        // Protocol narration, one line at a time. It is kept apart from onLog
        // because it is the same material the local backend puts on its debug
        // channel, and the page hides it behind the same toggle.
        this.onDebug = onDebug || function () {};
        ...
        // Ask the daemon for narration on every command from here on. Off by
        // default: the lines are a debugging aid, and a daemon sends none to a
        // request that did not ask.
        this.debug = false;
    ...
    async _command(command, payload) {
        ...
        /* Asks the daemon to interleave RESP_DEBUG frames into this response. It
         * is per request, so flipping the page's toggle reaches the next command
         * with no reconnect. */
        if (this.debug) headers['X-Debug'] = '1';
        ...
        for (;;) {
            ...
            if (status === RESP_PROGRESS) {
                ...
                if (body.length >= 4) {
                    ...
                    // A phase's opening frame carries only the phase's own name, so it
                    // is shown as that name rather than as "writing: writing".
                    const text = stage && msg && msg !== stage ? stage + ': ' + msg : (msg || stage);
                    this.onProgress(percent, text, stage);
                }
            } else if (status === RESP_LOG) {
                this.onLog(stripOneNewline(new TextDecoder().decode(body)));
            } else if (status === RESP_DEBUG) {
                /* Delivered whether or not this client asked for it: a narration
                 * frame is a line in the middle of a stream that still has its
                 * final frame behind it, so the one thing it must not do is end
                 * the command. Matched here rather than falling into the branch
                 * below, which reads any other status as an error and stops. */
                this.onDebug(stripOneNewline(new TextDecoder().decode(body)));
