 ...
 *
 * Drives the tdfu-wasm engine through ./tdfu.js and manages the UI state
 * machine. Every engine call is a plain promise, so there is no module mutex
 * and no staging a transfer through a virtual filesystem: the bytes go in and
 * come back as Uint8Arrays.
 */

import * as tdfu from './tdfu.js';
import { RemoteClient } from './remote.js';
import { DEFAULT_BRANCH, groupByBranch, menuBranchOf, releaseLabel } from './releases.js';
...
var detectedChip = '';        /* the chip as detected ("T31ZX"); '' until a bootrom answered */

/* The engine's handle for the device we are working on. Null when nothing is
 * attached. The local path used to pass a bare 0 to every C entry point, which
 * was only ever right because the shim held one device.
 *
 * What the engine promises: an id names one USBDevice *object* for as long as
 * the page holds it, and nothing more. It is NOT stable across a
 * re-enumeration - Chromium's USBDevice objects are keyed by a per-connection
 * guid, so the gadget that comes back after a bootstrap is a new object and the
 * engine interns it as a new id. The old id then names a dead handle that will
 * never work again. So every place a device re-enumerates re-discovers and takes
 * the id from the fresh DeviceInfo, and drops this one first. */
var deviceId = null;
...
/* One engine or daemon rejection, as a line for the log panel.
 *
 * The seam's rejection value is an Error carrying `message` and `kind`:
 * the message is tdfu-core's own wording and the kind is the
 * error variant's name, which is the only part stable enough for an operator to
 * look up or quote. The C had neither - it had an int code it turned into prose
 * at the call site - so showing the kind is a deliberate divergence, and a cheap
 * one: "device 0 is not in DFU mode [NotDfu]" says what failed and what class of
 * thing it was.
 *
 * Anything else that reaches a catch (a TypeError from the page's own code, a
 * bare string) still has to read as something, so fall back to what the value
 * can give rather than printing "undefined". */
function failureText(e) {
    if (!e) return 'unknown error';
    var msg = e.message || String(e);
    return e.kind ? msg + ' [' + e.kind + ']' : msg;
}
...
/* Does this release asset belong to that SoC?
 *
 * The SoC is a whole underscore-separated field of the asset name
 * (thingino-<vendor>_<model>_<soc>_<sensor>_<wifi>.bin), so the delimiters are
 * part of the test: `_t31_` must not match a `t31x` image. An unknown SoC
 * matches everything, because there is then nothing to contradict - a DFU
 * gadget picked up without a bootrom detection is exactly that case. */
function assetFitsSoc(name, soc) {
    if (!soc) return true;
    return new RegExp('_' + soc + '[._]').test(String(name).toLowerCase());
}

/* The SoC field of an asset name, spelled as the engine's own variant table
 * spells it, or '' when no field of the name is a variant. Used only to say
 * what a refused image was built for. */
function assetSoc(name) {
    var fields = String(name).toLowerCase().replace(/\.bin$/, '').split('_');
    var known = tdfu.variantNames();
    for (var i = 0; i < fields.length; i++)
        if (known.indexOf(fields[i]) !== -1) return fields[i];
    return '';
}
 ...
 *
 * It also requires the picked image to be built for the SoC that is attached.
 * A selection outlives the device it was made for: the list is filtered when it
 * is built, and a camera unplugged and replaced by another SoC leaves the old
 * image selected. Flashing it writes another SoC's U-Boot and DDR init. */
function updateReleaseFlash() {
    ...
    btn.disabled = !(currentState === 'done' && img && img.value &&
                     assetFitsSoc(img.value, detectedVariantName));
}

/* Rebuild the release picker against the device that is current now.
 *
 * The image list and the Flash button are only as good as the SoC they were
 * built against, and that SoC changes when a device is connected, detected,
 * bootstrapped, or picked on the daemon. Rebuilding at each of those points
 * drops a selection made for another camera instead of leaving it armed for
 * this one; nothing else re-runs the filter, so without this the panel keeps
 * showing the previous device's images. */
function releasesForCurrentDevice() {
    releaseChanged();
}
...
/**
 * Fetch the DFU-capable stage 1 + U-Boot for a variant, as bytes.
 *
 * The engine takes the two images, both or neither, so the page
 * fetches them and hands them over - there is no virtual filesystem to stage
 * them through any more, and no C-side path resolution to keep in step.
 *
 * The directory is the variant's own name: the engine reports the loader name
 * (`Variant::loader_dir`), which is what these directories are called, so the
 * hand-kept JS map the C repo replaced with a `tdfu_web_dfu_dir` export has no
 * successor here - there is nothing left to map.
 */
async function fetchDfuLoaders(variant) {
    // stage1 is tpl.bin on the capped XBurst1 SoCs (T10/T20/T21/T30) and spl.bin
    // on the big-SPL SoCs. The choice is by what is published, never by family:
    // 28 of the 34 loader directories ship a tpl.bin, so a 404 here is the
    // ordinary answer for the other six and we fall through to spl.bin.
    var stage1Name = 'tpl.bin';
    var s1 = await fetch('firmware/dfu/' + variant + '/tpl.bin');
    if (!s1.ok) {
        stage1Name = 'spl.bin';
        s1 = await fetch('firmware/dfu/' + variant + '/spl.bin');
    }
    if (!s1.ok) {
        throw new Error('Failed to fetch dfu stage1 for ' + variant + ': ' + s1.status);
    }
    var stage1 = new Uint8Array(await s1.arrayBuffer());
    console.log('Loaded dfu stage1 ' + stage1Name + ': ' + stage1.length + ' bytes');

    var ub = await fetch('firmware/dfu/' + variant + '/uboot.bin');
    if (!ub.ok) {
        throw new Error('Failed to fetch firmware/dfu/' + variant + '/uboot.bin: ' + ub.status);
    }
    var uboot = new Uint8Array(await ub.arrayBuffer());
    console.log('Loaded dfu uboot.bin: ' + uboot.length + ' bytes');
    return { spl: stage1, uboot: uboot };
}
...
/* A device's SoC, as far as it can be known at this stage.
 *
 * `info` is the engine's DeviceInfo, { id, vid, pid, stage, variant }, where
 * `stage` is classified from the descriptors rather than the PID - which is
 * what stops a DFU gadget sharing the bootrom's 0xC309 from being read as a
 * bootrom. Only a bootrom answers the three register reads; past that
 * the engine reports what it cached, or nothing. */
async function socNameFor(info) {
    if (info.stage !== 'bootrom') return { variant: info.variant || '', chip: '' };
    var d = await tdfu.detect(info.id);
    if (d.caveat) log(d.caveat, 'warn');
    return { variant: d.variant, chip: d.chip || '' };
}

/* What the device row shows for the SoC: the chip itself when detection named it
 * ("T31ZX"), else the loader variant, else the placeholder. The CLI's longer form
 * ("T31ZX (loader t31x, DDR2)") does not fit the row. */
function socLabel() {
    return detectedChip || (detectedVariantName ? detectedVariantName.toUpperCase() : '—');
}
...
/* One log line from the engine. The level arrives with it, so there is no
 * prefix to sniff, and byte counts are not log lines any more - they come
 * through onEngineProgress as numbers. */
function routeEngineLog(text, level) {
    log(text, level || 'info');
}

/* The engine names its phases in plain words rather than the DFU protocol's (where an
 * "upload" moves bytes from the device to the host and a "download" the other way);
 * the bar keeps the fuller labels the shipped flasher used, and a phase not listed
 * here shows as its own name. */
var PHASE_LABELS = {
    'reading': 'Reading flash via DFU...',
    'writing': 'Writing flash via DFU...',
    'finishing': 'Finishing the write via DFU...',
    'verifying': 'Verifying flash via DFU...',
    'erasing': 'Erasing flash via DFU...',
    'stage1': 'USB-booting U-Boot (DFU)...',
    'u-boot': 'USB-booting U-Boot (DFU)...'
};

/* One progress frame from the engine: the phase it is in, bytes done, and the
 * total when there is one. A whole-chip read ends on a short block, so it has
 * no knowable total, and draws the striped bar with a live byte
 * count instead of a percentage. */
function onEngineProgress(phase, done, total) {
    var mb = (done / 1048576).toFixed(1);
    var label = (PHASE_LABELS[phase] || phase) + ' ' + mb +
        (total ? ' / ' + (total / 1048576).toFixed(1) : '') + ' MB';
    // The striped bar for the whole operation, as the shipped page drew it. An
    // operation is several phases (stage 1 then U-Boot, a write then its verify), and
    // a percentage per phase runs the bar to the end and back to the start for each
    // one, so the bar stays the one the page always had. The count in the label is
    // what the engine adds. Completion sets 100% ("Read complete", ...).
    showProgressBusy(label);
}

async function initEngine() {
    console.log('Loading the flasher engine...');

    try {
        var banner = await tdfu.initTdfu({ log: routeEngineLog, progress: onEngineProgress });

        // Verbose engine diagnostics, off unless the Settings toggle is on.
        if (debugEnabled) {
            tdfu.setDebug(true);
        ...
        // The engine's banner already carries its version and the commit it was
        // built from, and the page bundle is content-hashed beside it, so the
        // two cannot drift the way the fixed-name glue could. __GIT_SHA__ is the
        // fallback that still identifies the page when the engine did not load.
        var verEl = document.getElementById('version-num');
        if (verEl) verEl.textContent = 'v' + banner;
        ...
        var el = document.getElementById('version-num');
        var sha = typeof __GIT_SHA__ !== 'undefined' ? __GIT_SHA__ : 'dev';
        if (el) el.textContent = 'page ' + sha + ', no engine';
        console.error(e);
    }
}
...
async function connectDevice() {
    ...
    try {
        // The chooser and its Ingenic filters (bootrom, firmware and the DFU
        // gadget, so re-enumeration keeps the grant) belong to the engine now.
        // It resolves null when the user dismisses the chooser.
        var info = await tdfu.requestDevice();
        if (!info) {
        ...
        console.log('Device selected: VID=0x' + info.vid.toString(16) +
            ' PID=0x' + info.pid.toString(16) + ' (' + info.stage + ')');
        deviceId = info.id;
        ...
        // same "T31V" as the T31), so a bootrom device gets the three register
        // reads and whatever caveat they come with.
        var probed = await socNameFor(info);
        detectedVariantName = probed.variant;
        detectedChip = probed.chip;

        var stageName = info.stage === 'bootrom' ? 'Bootrom' : 'Firmware';
        var soc = socLabel();
        showDeviceInfo(soc, stageName, info.vid, info.pid);
        log('Detected: ' + soc + ' (' + stageName + ')');

        setState('done');
        releasesForCurrentDevice();
    } catch (e) {
        log('Connection error: ' + failureText(e), 'error');
        console.error(e);
        setState('error');
    }
}
 ...
 * gesture. Driven by the USB 'connect' event, and by nothing else: there is no
 ...
 * all unless it is caught here. It also sets 'detecting' before its first await,
 * and 'detecting' disables Connect, Bootstrap, Write, Read, Info and Settings -
 * so an uncaught rejection used to leave every control greyed and the log panel
 * silent. The C could not hit this: its discoverDevices() answered
 * null on a failure code rather than rejecting. */
...
async function autoAttachDevice(device) {
    ...
    /* Never hijack an operation in flight (read/write/bootstrap). 'error' is not
     * one: every catch in this file sets it after its await has already settled,
     * so nothing is running. The C skipped it too and left a failed page unable
     * to pick up a re-plug at all; here the next hotplug is exactly the retry the
     * user was told to make, so let it through. */
    ...
    setState('detecting');
    try {
        // The event carries a USBDevice; the engine's list is what we work from,
        // so find the arrival in it. Its id is a NEW id: a re-enumerated device
        // is a new USBDevice object, so the engine interns it as a new handle
        // and the id that named this camera before the bootstrap names nothing
        // now.
        //
        // Which of several matching entries is the arrival is decided by the id,
        // never by where it sits in the list. The engine interns by object
        // identity and appends, so an object it has not seen before - which a
        // re-enumerated device always is - carries the largest id of any entry
        // that matches. getDevices() order is the browser's and says nothing
        // about arrival, so with two authorized cameras of one model the last
        // list element is the other camera as often as it is this one, and every
        // later Bootstrap, Write and Read in the session would go to it.
        var all = await tdfu.discover();
        var matches = all.filter(function (d) {
            return d.vid === device.vendorId && d.pid === device.productId;
        });
        var info = matches.length
            ? matches.reduce(function (best, d) { return d.id > best.id ? d : best; })
            : null;
        if (!info) { log('autoAttach: the engine does not see it', 'debug'); setState('idle'); return; }
        deviceId = info.id;
        ...
        // Say what happened and hand the controls back. Connect is live in
        // 'error', so the manual pick is one click away.
        log('Auto-attach failed: ' + failureText(e), 'error');
        log('Click Connect Device to pick it by hand.', 'warn');
        console.error(e);
        setState('error');
    }
}
...
/* An engine that failed to load leaves these three buttons doing nothing. Say so
 * rather than returning in silence: connectDevice already did, and
 * the footer reads "page <sha>, no engine", but a button that no-ops with an
 * empty log panel is indistinguishable from a hung one. Copied-verbatim silence
 * from the C, corrected here. */
async function doBootstrap() {
    if (backendMode === 'remote') return doRemoteBootstrap();
    if (!tdfuReady) { log('Engine not ready', 'warn'); return; }
...
async function doWrite() {
    ...
    if (backendMode === 'remote') {
        if (firmwareData) return doRemoteWrite(firmwareData);
        log('Select a firmware file first', 'warn');
        return;
    }
    if (!tdfuReady) { log('Engine not ready', 'warn'); return; }
...
async function applyOverlayInjection() {
    ...
    /* Building the files can refuse: a passphrase carrying a character
     * wpa_supplicant.conf has no spelling for is an abort, not a truncation. */
    var files;
    try {
        files = overlayFilesFor({ ssid: ssid, psk: psk, sshKey: sshKey });
    } catch (e) {
...
async function doDfuBootstrap() {
    ...
    if (deviceId === null) { log('Connect a device in bootrom mode first', 'warn'); return; }
    if (!detectedVariantName && !custom) { log('Connect a device in bootrom mode first', 'warn'); return; }
    ...
    try {
        // Both images or neither: the custom pair when the Advanced
        // panel has one loaded, otherwise the bundled pair for this SoC.
        var images = custom
            ? { spl: customSpl.data, uboot: customUboot.data }
            : await fetchDfuLoaders(detectedVariantName);
        showProgress(40, custom ? 'USB-booting custom U-Boot (DFU)...' : 'USB-booting U-Boot (DFU)...');
        await tdfu.bootstrap(deviceId, {
            variant: custom ? undefined : detectedVariantName,
            spl: images.spl,
            uboot: images.uboot,
        });
        showProgress(100, 'U-Boot DFU running');
        /* The bootrom is gone and its id with it: the gadget enumerates as a new
         * USBDevice, so the engine issues a new id for it. Drop this
         * one now rather than carrying a dead handle through the wait - the
         * 'connect' listener's discover() below is what supplies the new one. */
        deviceId = null;
        ...
        log('DFU bootstrap error: ' + failureText(e), 'error');
...
async function doDfuRead() {
    ...
    log('Reading flash via DFU...');
    try {
        // No size: the whole alt. A DFU upload ends on a short block, so the
        // engine reports bytes with no total and the bar stays indeterminate.
        var data = await tdfu.read(deviceId, {});
        ...
        log('DFU read error: ' + failureText(e), 'error'); console.error(e); hideProgress(); setState('error');
    }
}
...
async function doDfuWrite() {
    ...
    var image = firmwareData;
    showProgressBusy('Writing flash via DFU...');
    log('Writing ' + firmwareFileName + ' (' + image.length + ' bytes) to flash via DFU...');
    try {
        // One call, so the read-back compares against exactly the bytes that were
        // written: the engine does the download and, with verify, reads the flash
        // back itself. There is no temporary file for either half to disagree on.
        await tdfu.write(deviceId, { image: image, verify: doVerify });
        log('Firmware written via DFU!');
        showProgress(100, doVerify ? 'Write + verify complete' : 'Write complete');
        if (doVerify) log('Verify OK: flash matches');
        if (doReboot) {
            log('Rebooting the device...');
            try { await tdfu.reboot(deviceId); log('Reboot triggered'); }
            catch (e) { log('Reboot failed: ' + failureText(e), 'warn'); }
        }
        setTimeout(hideProgress, 1500); setState('done');
    } catch (e) {
        /* A verify mismatch is not a failed download: the bytes went, and the
         * read-back disagreed. The C said which it was in its own
         * wording ("DFU verify FAILED ... flash does not match"); one call now
         * does both passes, so the prefix branches on the rejection's kind, which
         * is the seam's stable name for it, never on the message text. */
        var verifyFailed = !!(e && e.kind === 'Verify');
        log((verifyFailed ? 'DFU verify failed: ' : 'DFU write error: ') + failureText(e), 'error');
        console.error(e); hideProgress(); setState('error');
    }
}
...
async function doDiag() {
    ...
    try {
        if (backendMode === 'remote') {
            ...
            if (deviceId === null) { log('Connect a device first', 'warn'); return; }
            log('Reading device info (eFuse)...');
            text = await tdfu.diag(deviceId);
        }
    } catch (e) {
        log('Info error: ' + failureText(e), 'error'); console.error(e);
    }
    // A failed readout now rejects rather than returning prose, so there is no
    // "Diag failed" sentinel to sniff for: text or nothing.
    var ok = !!(text && text.length);
    if (ok) {
        lastDiagText = text;
...
function setDebug(on) {
    ...
    /* Takes effect at once, mid-transfer included: the engine's setter is a
     * plain synchronous call, not an entry into a module that can only be
     * inside itself once. (The old one silently did nothing until a reload.) */
    tdfu.setDebug(debugEnabled);
    /* The daemon backend asks per request, so a connected client carries this
     * into its next command with nothing to reconnect. */
    if (remoteClient) remoteClient.debug = debugEnabled;
...
/* ------------------------------------------------------------------ */
/*  Remote daemon backend (dfu-remote over HTTP)                       */
/* ------------------------------------------------------------------ */
...
async function doRemoteConnect() {
    ...
    var client = new RemoteClient(
        // Whole log lines, already stripped of their frame's terminator by
        // remote.js. Byte counts do not come this way any more: this daemon
        // sends RESP_PROGRESS frames, which land below.
        function(m) { log(m, 'info'); },
        // A progress frame. The message carries the stage and the live byte count;
        // the bar stays the striped one for the whole operation, as on the local
        // path (a percentage per phase would run it forwards and back for each of
        // a write's phases). Completion sets 100%.
        function(p, msg) {
            showProgressBusy(msg || '');
        },
        // Resolve a DISCOVER ordinal to its name through the engine's copy of the
        // frozen table. Never a hardcoded JS table: that drifts as
        // per-variant loaders are added and mis-bootstraps the SoC.
        function(v) { return tdfu.variantName(v); },
        // The daemon's protocol narration: GETSTATUS poll rounds, alt claiming,
        // descriptor lines, one per downloaded block. It is the same material the
        // local backend puts on the engine's debug channel, so it renders at the
        // same level and behind the same toggle.
        function(m) { log(m, 'debug'); }
    );
    // Narration is a frame per line, so the daemon is only asked for it while the
    // toggle is on. A later flip reaches this same client through setDebug().
    client.debug = debugEnabled;
    try {
        await client.connect(remoteUrl, remoteToken || null);
    } catch (e) {
        log('Connection failed: ' + failureText(e), 'error');
    ...
    var devs = [];
    try {
        devs = await client.discover();
        /* null is a refusal, not an empty bus; the daemon's own ERROR line is
         * already in the log above. Fall into the same handling a thrown
         * failure gets rather than into "no devices found". */
        if (!devs) throw new Error('the daemon refused the device listing');
    } catch (e) {
        /* A refusal is not an empty list. The commonest remote failure is a wrong
         * --token, which this daemon answers with a 403 the browser can actually
         * read (the daemon puts CORS on its refusals); falling through to the
         * empty-list branch below then told the user "No Ingenic devices found on
         * the daemon", which is the opposite of the truth. Drop the
         * client too, so the next Connect retries instead of disconnecting. */
        log('Discover failed: ' + failureText(e), 'error');
        log('Check the daemon URL and token in Settings, then Connect again.', 'warn');
        remoteClient = null;
        remoteDevices = [];
        document.getElementById('device-info').classList.add('d-none');
        document.getElementById('device-disconnected').classList.remove('d-none');
...
async function doRemoteBootstrap() {
    ...
    try {
        ...
        for (var i = 0; i < 20 && !d; i++) {
            ...
            try {
                /* A refusal mid-poll is answered like a failed one: keep waiting
                 * for the gadget rather than treating null as a list. */
    ...
    } catch (e) { log('Remote bootstrap error: ' + failureText(e), 'error'); hideProgress(); setState('error'); }
}
...
async function doRemoteRead() {
    ...
    try {
        ...
        setTimeout(hideProgress, 1500);
        setState('done');
    } catch (e) { log('Remote read error: ' + failureText(e), 'error'); hideProgress(); setState('error'); }
}
...
async function doRemoteWrite(data) {
    ...
    try {
        ...
        if (doReboot) {
            ...
            try {
                var rebooted = await remoteClient.reboot(selectedRemoteIndex);
                log(rebooted ? 'Reboot triggered' : 'The daemon refused the reboot; see the ERROR line above.',
                    rebooted ? 'info' : 'warn');
            }
            catch (e) { log('Reboot failed: ' + failureText(e), 'warn'); }
        }
        setTimeout(hideProgress, 1500);
        setState('done');
    } catch (e) { log('Remote write error: ' + failureText(e), 'error'); hideProgress(); setState('error'); }
}
...
var relBranches = { branches: [], byBranch: {} }; /* the same listing, grouped (releases.js) */
var relLoaded = false;
/* Which branch's releases the picker shows: `master` by default, and
 * the choice is remembered like the other settings. */
var relBranch = localStorage.getItem('tdfu_rel_branch') || DEFAULT_BRANCH;
...
/* (Re)build the branch dropdown from the grouped listing, keeping the remembered
 * branch when the listing still has it and falling back to the first one (the
 * stable line whenever it exists). */
function renderBranchOptions() {
    var sel = document.getElementById('rel-branch');
    if (!sel) return;
    sel.innerHTML = '';
    relBranches.branches.forEach(function(branch) {
        sel.appendChild(new Option(branch, branch));
    });
    if (relBranches.branches.indexOf(relBranch) === -1) relBranch = relBranches.branches[0] || '';
    sel.value = relBranch;
    sel.disabled = relBranches.branches.length === 0;
}

/* (Re)build the release dropdown for the chosen branch, preserving the selection.
 * Split out of loadReleases so a language switch or a branch change can rebuild
 * the options without hitting the API again. Newest first, as the API lists them. */
function renderReleaseOptions() {
    ...
    (relBranches.byBranch[relBranch] || []).forEach(function(entry) {
        sel.appendChild(new Option(releaseLabel(entry, t('rel_prerelease')), entry.tag));
    });
    sel.value = keep;
}

/* The branch menu changed: remember it, list that branch's releases, and reset the
 * device list, which belonged to a release of the old branch. */
function branchChanged() {
    var sel = document.getElementById('rel-branch');
    relBranch = sel ? sel.value : DEFAULT_BRANCH;
    localStorage.setItem('tdfu_rel_branch', relBranch);
    var rel = document.getElementById('rel-release');
    if (rel) rel.value = '';
    renderReleaseOptions();
    releaseChanged();
}

/* List the releases of the two branches the menu offers (releases.js: `master-`
 * builds and the stable `firmware-` line built from `ciao`). api.github.com sends
 * ACAO:*, so this is a plain cross-origin fetch - no proxy involved. 100 is the API's
 * page maximum; with several branches publishing, 30 no longer reached back far
 * enough. */
async function loadReleases() {
    var sel = document.getElementById('rel-release');
    try {
        var r = await fetch('https://api.github.com/repos/themactep/thingino-firmware/releases?per_page=100');
        if (!r.ok) throw new Error('HTTP ' + r.status);
        var rels = (await r.json()).filter(function(x) {
            return !x.draft && menuBranchOf(x.tag_name);
        });
        relAssets = {};
        rels.forEach(function(x) { relAssets[x.tag_name] = x.assets; });
        relBranches = groupByBranch(rels);
        relLoaded = true;
        renderBranchOptions();
        renderReleaseOptions();
        log('Found ' + rels.length + ' thingino firmware releases on ' +
            relBranches.branches.length + ' branch' + (relBranches.branches.length === 1 ? '' : 'es') + '.');
    } catch (e) {
        sel.innerHTML = '';
...
function releaseChanged() {
    ...
    var list = bins;
    if (soc && !all) list = bins.filter(function(a) { return assetFitsSoc(a.name, soc); });
    ...
    /* A browser <select> drops a value it has no option for; do it here too, so
     * the rule holds wherever the page runs and so the operator is told that the
     * image they picked went away with the device it was picked for. */
    var offered = list.some(function(a) { return a.name === keep; });
    dev.value = offered ? keep : '';
    if (keep && !offered)
        log('Cleared the selected image ' + keep + ': this release list no longer offers it.', 'warn');
    updateReleaseFlash();
 ...
 *
 * Fails closed. These bytes are about to be written to a camera's flash and
 * this is the only check on them: downloadWithProgress compares nothing, and a
 * truncated or substituted image bricks the box until someone flashes it again
 * over USB. So a sums file that is missing, unreadable or cannot be parsed, and any
 * error on the way, refuse the flash. A warn line in a log nobody is watching
 * during a download is not a decision the operator made, and there is no
 * "flash it unverified" control in this page for them to make it with. */
async function verifySha256(tag, name, data) {
    try {
        ...
        if (!r.ok) {
            log('No sha256sum published for ' + name + ' (HTTP ' + r.status +
                '); the image was NOT verified, so it will not be flashed.', 'error');
            return false;
        }
        ...
        if (!want) {
            log('Could not parse the published sha256sum for ' + name +
                '; the image was NOT verified, so it will not be flashed.', 'error');
            return false;
        }
        ...
        log('SHA256 check failed: ' + e.message +
            '; the image was NOT verified, so it will not be flashed.', 'error');
        return false;
    }
}
...
async function flashFromRelease() {
    ...
    /* The image carries this SoC's U-Boot and DDR init, so one built for another
     * SoC leaves the camera unable to boot. The button is gated on this too, and
     * the list is filtered when it is built, but neither survives a device
     * swapped under a made selection - and this is the last point before the
     * bytes are fetched and written. */
    if (!assetFitsSoc(name, detectedVariantName)) {
        var built = assetSoc(name);
        log('Refusing to flash ' + name + ': it is built for ' +
            (built ? built.toUpperCase() : 'another SoC') +
            ' and the connected device is ' + detectedVariantName.toUpperCase() + '.', 'error');
        updateReleaseFlash();
        return;
    }
...
(function() {
    ...
    navigator.usb.addEventListener('connect', function(e) {
        ...
        // Re-attach automatically if we already have permission for it. The
        // .catch is belt and braces: autoAttachDevice handles its own failures,
        // and a listener is the one place where a promise nobody holds can
        // reject into the void if it ever stops doing so.
        autoAttachDevice(e.device).catch(function (err) {
            log('Auto-attach failed: ' + failureText(err), 'error');
            console.error(err);
            setState('error');
        });
    });
    navigator.usb.addEventListener('disconnect', function(e) {
        ...
        // The engine keeps the device list; nothing to unregister here. A device
        // that leaves mid-operation surfaces as a rejected transfer, which is
    ...
    if (window.I18N) {
        ...
        window.addEventListener('i18nchange', function () {
            ...
            if (relLoaded) { renderBranchOptions(); renderReleaseOptions(); releaseChanged(); }
        });
    }
