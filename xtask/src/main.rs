...
/// How many times one asset is asked for before the fetch gives up.
///
/// A release job fetches 68 assets in a row from GitHub's CDN, and one reset connection
/// among them failed a whole tag run (v2.0.0-rc.2, `package (android)`: `Connection
/// reset by peer` on the second asset). Three tries with a short pause between cure
/// that; a failure that is not the network's is not retried at all.
const DOWNLOAD_ATTEMPTS: u32 = 3;

/// Is an HTTP status one a retry can cure? A server error or a rate limit, yes; a 404 or
/// a 403 says the same thing on the next try.
const fn retryable_status(code: u16) -> bool {
    code >= 500 || code == 429
}

/// Why one attempt failed, and whether another is worth making.
enum Fetch {
    /// The network's fault, or a 5xx/429: try again.
    Transient(String),
    /// The answer is the answer.
    Final(String),
}

/// GET `url`, following redirects, with [`DOWNLOAD_ATTEMPTS`] tries for a failure the
/// network caused. Fails loudly: there is no offline fallback, because a silently absent
/// loader tree is a bootstrap that fails on hardware for no visible reason.
fn download(url: &str) -> Result<Vec<u8>, Box<dyn std::error::Error>> {
    let mut last = String::new();
    for attempt in 1..=DOWNLOAD_ATTEMPTS {
        match download_once(url) {
            Ok(body) => return Ok(body),
            Err(Fetch::Final(reason)) => return Err(reason.into()),
            Err(Fetch::Transient(reason)) => {
                last = reason;
                if attempt < DOWNLOAD_ATTEMPTS {
                    eprintln!("xtask: {last}; attempt {attempt} of {DOWNLOAD_ATTEMPTS}, retrying");
    ...
    Err(format!(
        "{last} after {DOWNLOAD_ATTEMPTS} attempts - the loaders are downloaded, never vendored (D2), so there \
...
/// One GET, classified.
fn download_once(url: &str) -> Result<Vec<u8>, Fetch> {
    ...
    let mut response = match request.call() {
        ...
        Err(ureq::Error::StatusCode(code)) if retryable_status(code) => {
            return Err(Fetch::Transient(format!("GET {url} -> HTTP {code}")));
        }
        Err(ureq::Error::StatusCode(code)) => return Err(Fetch::Final(format!("GET {url} -> HTTP {code}"))),
        Err(e) => return Err(Fetch::Transient(format!("GET {url}: {e}"))),
    };
    let status = response.status();
    if !status.is_success() {
        let reason = format!("GET {url} -> HTTP {status}");
        return Err(if retryable_status(status.as_u16()) {
            Fetch::Transient(reason)
        } else {
            Fetch::Final(reason)
        });
    }
    let mut body = Vec::new();
    // A body cut short is the connection dropping mid-transfer: the network's fault.
    response
        .body_mut()
        .as_reader()
        .take(DOWNLOAD_LIMIT)
        .read_to_end(&mut body)
        .map_err(|e| Fetch::Transient(format!("GET {url}: reading the body: {e}")))?;
    if body.is_empty() {
        return Err(Fetch::Final(format!("GET {url} returned an empty body")));
    }
    Ok(body)
}
...
#[cfg(test)]
mod tests {
    /// A retry cures a reset, a 5xx and a rate limit, and nothing else: a 404 on a loader
    /// asset is the release being wrong, and asking three times would only hide it.
    #[test]
    fn only_the_networks_failures_are_retried() {
        assert!(super::retryable_status(500));
        assert!(super::retryable_status(503));
        assert!(super::retryable_status(429));
        assert!(!super::retryable_status(404));
        assert!(!super::retryable_status(403));
        assert!(!super::retryable_status(401));
        assert_eq!(super::DOWNLOAD_ATTEMPTS, 3);
    }
