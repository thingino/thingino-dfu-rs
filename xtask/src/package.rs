...
fn help_lines(root: &Path, stage: &Path, target: Target) -> HelpLines {
    ...
    HelpLines {
        cli: from_binary("thingino-dfu")
            ...
            .unwrap_or_else(|| "Flash Ingenic XBurst devices over USB using DFU.".to_owned()),
...
fn render_readme(facts: &Readme) -> String {
    ...
    let _ = writeln!(
        out,
        "Flash Ingenic XBurst devices (T10 to T41, A1) over USB using DFU. Built from {hash}.\n"
    );
...
"#;
        assert_eq!(
            ...
            Some("Flash Ingenic XBurst devices over USB using DFU.")
        );
    }
...
";
    ...
    fn facts(target: Target) -> Readme {
        Readme {
            ...
            help: Some(HelpLines {
                cli: "Flash Ingenic XBurst devices over USB using DFU.".to_owned(),
    ...
    fn the_readme_carries_the_version_the_pin_and_both_help_lines() -> TestResult {
        ...
        assert!(
            text.contains("Flash Ingenic XBurst devices over USB using DFU."),
            "the cli help"
        );
        assert!(
